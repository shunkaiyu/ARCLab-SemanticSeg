...
# def extractFrame(videoAddress,saveAddress):
#     video = cv2.VideoCapture(videoAddress)
#     videoId = (videoAddress.split('.')[-2]).split('_')[-1]
...
#         if success:
#             if frameNum % 30 == 0:
#                 cv2.imwrite(f'{saveAddress}/{videoId}_frame_{frameNum}_ch2.png',frame)
#         else:
#             break
#         frameNum = frameNum + 1
...
# videoDir = './'
# storeDir = './frames'
# video_list = np.array([f for f in os.listdir(videoDir) if (f.endswith(".mp4"))])


def predictVideo(model,key,videoAddress,outAddress):
    ...
    video = cv2.VideoCapture(videoAddress)
    videoName = (videoAddress.split('.')[-2]).split('/')[-1]
    fps = video.get(cv2.CAP_PROP_FPS)
    out = cv2.VideoWriter(f'{outAddress}/{videoName}_seg.mp4',cv2.VideoWriter_fourcc(*'MJPG'),30,(848*2,480))
    frameNum = 0
    while (True):
        success, frame = video.read()
        frameNum = frameNum + 1
        if success:
            ...
            im_ori = cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)

            im_ori = Image.fromarray(im_ori)
            ...
            im_ori = im_ori.resize((848,480))
            
            
            im_ori = np.array(im_ori)
            im_ori = cv2.cvtColor(im_ori, cv2.COLOR_BGR2RGB)

            result = np.concatenate([im_ori,seg],axis=1)


            out.write(result)
            # cv2.imwrite(f'{outAddress}/test.png', result)
...
def main():
    ...
    videoDir = "/home/arcseg/Desktop/Shunkai-working/src/data/video_data"
    video_list = np.array([f for f in os.listdir(videoDir) if (f.endswith(".mp4"))])
    ...
    for name in video_list:
        ...
        predictVideo(model,key,f'{videoDir}/{name}',f'{videoDir}')
        print(f"Inference {name} Complete!")
