...
from data.dataloaders.SegNetDataLoaderV2T import SegNetDataset # V2 is for loading entire dataset into memory, considering cholec data can easily fit into memory
...
def main():
    ...
    if args.model == 'segnet':
        ...
        model = model.to(device)
    elif args.model == "smp_DeepLabV3+T":
        model = DeepLabV3Plus(
...
def train(train_loader, model, criterion, dice_loss, focal_loss, optimizer, scheduler, epoch, key, losses, img_mean, img_std, logger, args):
    ...
    for i, (img, gt, label, globleimg, localimg) in train_loop:
        #print(img.size(0))
